...
export const MyDiplomas = () => {
    ...
    const [diplomasList, setDiplomasList] = useState<any[]>([]);
    ...
    const { data: diplomaContract } = useScaffoldContract({
        contractName: "DiplomaNFT",
    });
            ...
            {!balance || balance.toString() === "0" ? (
                <div className="bg-secondary p-12 rounded-3xl text-center border-2 border-dashed border-primary/30">
                    ...
                    <p className="opacity-70 max-w-md mx-auto mb-6">
                        Completa un curso y espera a que la institución emita tu NFT.
                        ¡Tus logros aparecerán aquí de forma inmutable!
                    </p>
                    <div className="bg-base-300 p-4 rounded-xl text-left text-sm max-w-lg mx-auto border border-primary/10">
                        ...
                        <p>
                            A diferencia de un PDF, un diploma NFT te pertenece a ti (a tu wallet)
                            y nadie puede quitártelo ni falsificarlo.
                        </p>
                    </div>
                </div>
            ) : (
